...
$st = strtotime($matches[2] . '/' . $matches[3] . '/' . $matches[1]);
$a = date('m', $st);
$b = date('j', $st);
$c = date('Y', $st);

preg_match('/^([0-9]{4})([0-9]{2})([0-9]{2})$/', $end, $matches);
$d = $matches[2];
$e = $matches[3]; 
$f = $matches[1];

$url = 'http://ichart.finance.yahoo.com/table.csv?s=' . $symbol . 
	'&a=' . ($a - 1) . '&b=' . $b . '&c=' . $c . '&d=' . ($d - 1) . '&e=' . $e . '&f=' . $f . '&g=d&ignore=.csv';
...
foreach ($arr_data as $row) {
	list($d, $o, $h, $l, $c, $v, $a) = explode(",", $row);
	$d1 = substr($d, 5, 2);
	$d2 = substr($d, 8, 2);
	$d3 = substr($d, 0, 4);
	$factor = $a / $c;
...
////S,Date,Vol,High,Low,Open,Close
echo "<pre>"; print_r($arr_data); echo "</pre>"; 

echo "<pre>"; print_r(SMA($arr_data, 10, 6, 2)); echo "</pre>";
echo "<pre>"; print_r(EMA($arr_data, 10, 6, 2)); echo "</pre>";
echo "<pre>"; print_r(SMABody($arr_data, 10, 5, 6, 2)); echo "</pre>";

echo "<pre>"; print_r(Candles($arr_data, 5, 6, 3, 4)); echo "</pre>";

//function Candles ($array_ref, $index_open, $index_close, $index_high, $index_low) {
//return array($CandleColor, $CandleTrend, $CandlePatterns);
exit;
...
function Candles ($array_ref, $index_open, $index_close, $index_high, $index_low) {
	...
	for ($i = 0; $i < count($array_ref); $i++) {
		...
		//my ($trend, $body_type, $body, $base_body, $high_low, $pattern, $body_day, $body_mid);
		...
		$body_type = "Short";
		if ($body >= ($sma_body[$i] * 0.750)) { $body_type = "Long"; }
		
		
		// body less than 20% of sma body
		//elseif ($body <= ($sma_body->[$i] * 0.10)) { $body_type = "Doji";	}
		elseif ($body <= ($sma_body[$i] * 0.10) || $lower_shadow > 490 || $upper_shadow > 490) { $body_type = "Doji";	}
		...
		if ($color == 'White' && 
			...
			$pattern = 'Engulging R+';
			array_push($patterns, 'Engulging R+');
		}
		...
		//61 latter bottom + (JNY 1996-12-18)
		if ($color == 'White' &&
			$hist_color[1] == 'Black' &&
			$hist_color[2] == 'Black' &&
			$hist_color[3] == 'Black' &&
			...
			$pattern = 'Latter Bottom R+';
			array_push($patterns, 'Latter Bottom R+');
		}
		
		//62 latter top - AMR 1998-09-24 
		if ($color == 'Black' &&
			$hist_color[1] == 'White' &&
			$hist_color[2] == 'White' &&
			$hist_color[3] == 'White' &&
			...
			$pattern = 'Latter Top R-';
			array_push($patterns, 'Latter Top R-');
		}
...
/*##################################################################
##
## SMA: calculate simple moving average of Yahoo finance format 
##      historical stock data
##
## Usage:  $sma = Candles::SMA(@yahoo_csv_data, $sma_days, $index_of_close_amount, $decimal_places);
##
## (note: Each element of array is CSV delimited data with ascending date)
##
##        Date,Open,High,Low,Close,Volume,Adj_Volume
##
## Returns: ref to SMA array containing same count as original 
##
###################################################################*/
